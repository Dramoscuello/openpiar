...
async def test_medical_support_pdf_export_import_cycle():
    async with AsyncSessionLocal() as session:
        ...
        # 2. Añadir Entorno Salud con PDF de soporte médico ficticio
        ...
        # Verificar que el PDF esté serializado en Base64
        ...
        # 4. Deserializar e importar en un nuevo registro (simulando otro colegio)
        # Cambiamos el documento de identidad en el JSON para evitar conflicto de unicidad
        ...
        # 5. Cargar estudiante importado de la base de datos
        stmt = select(EstudianteORM).where(EstudianteORM.id == imported_student.id)
        res = await session.execute(stmt)
        student_db = res.scalars().first()
        
        # Verificar que el entorno de salud importado tenga el PDF correcto
        ...
        # Forzar rollback al final del test para no alterar la base de datos real
